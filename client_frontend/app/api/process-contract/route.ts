...
export async function POST(req: NextRequest) {
  try {
    ...
    if (adminEmail) {
      ...
      const admin = adminGenericFormEmail("Contracting Opportunity", data, "fromemail");
      ...
      if (userEmail) {
        const user = userGenericFormEmail("Contracting Opportunity", data, "contactpreson");
        emailPromises.push(
          transporter.sendMail({
            from: fromAddress,
            to: userEmail,
      ...
      await Promise.all(emailPromises).catch(err => console.error("Error sending contracting opportunity emails:", err));
    } else {
    ...
    const result = await db.collection('contracts').insertOne(submission);

    return NextResponse.json({
      success: true,
      id: result.insertedId,
      message: "Contracting Opportunity submitted successfully",
    });
  } catch (error: any) {
    console.error('Error saving contracting opportunity:', error);
    return NextResponse.json(
      { error: 'Failed to submit contracting opportunity' },
      { status: 500 }
    );
  }
}
