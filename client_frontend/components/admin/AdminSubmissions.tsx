                  ...
                  <tbody>
                    {filteredSubmissions.map((sub) => {
                      ...
                      const primaryService =
                        sub.services?.[0] || "General Inquiry";
          ...
          <div className="bg-[#131e35] w-full max-w-[720px] rounded-[20px] overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[85vh] animate-fade-up">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModalSection title="Client Contact">
                  <DetailRow
                    label="Name"
                    value={`${selectedSubmission.name}${selectedSubmission.job_title ? ` (${selectedSubmission.job_title})` : ""}`}
                  />
                  ...
                  {selectedSubmission.address && (
                    <DetailRow
                      label="Address"
                      value={`${selectedSubmission.address}, ${selectedSubmission.city}, ${selectedSubmission.statezip}`}
                    />
                  )}
                </ModalSection>
                <ModalSection title="Site & Requirements">
                  {selectedSubmission.jobsite_location && (
                    <DetailRow
                      label="Job Site"
                      value={`${selectedSubmission.jobsite_location}, ${selectedSubmission.city2}, ${selectedSubmission.statezip2}`}
                    />
                  )}
                  {selectedSubmission.svctype1 && (
                    <DetailRow
                      label="Officer Type"
                      value={selectedSubmission.svctype1}
                    />
                  )}
                  {selectedSubmission.svctype2 && (
                    <DetailRow
                      label="Appearance"
                      value={selectedSubmission.svctype2}
                    />
                  )}
                  {selectedSubmission.guards_needed && (
                    <DetailRow
                      label="Guards"
                      value={selectedSubmission.guards_needed}
                    />
                  )}
                  {selectedSubmission.permanent && (
                    <DetailRow
                      label="Duration"
                      value={selectedSubmission.permanent}
                    />
                  )}
                </ModalSection>
              </div>
              ...
              {selectedSubmission.services &&
                selectedSubmission.services.length > 0 && (
                  <div className="mt-5">
                    <ModalSection title="Services">
