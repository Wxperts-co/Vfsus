"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import {
  LogOut,
  Search,
  RefreshCw,
  FileText,
  CheckCircle,
  Clock,
  AlertCircle,
  Shield,
  X,
  Eye,
  LayoutDashboard,
  TrendingUp,
  Users,
  Activity,
  Trash2,
} from "lucide-react";
import AdminSidebar from "./AdminSidebar";

interface ContractSubmission {
  _id: string;
  contactpreson: string;
  dba?: string;
  company: string;
  moaddress: string;
  city: string;
  state: string;
  zip: string;
  ophone: string;
  fax?: string;
  fromemail: string;
  website: string;
  stservice: string;
  coservice: string;
  bcbackground: string;
  bdpresident: string;
  services?: string[];
  orgtypetext?: string;
  orgstruct?: string;
  fein?: string;
  ssle?: string;
  inscarrier?: string;
  insagent?: string;
  insphone?: string;
  comliab: string;
  contact?: string;
  vendor?: string;
  creditscale?: string;
  voilation: string;
  judgment: string;
  recruiting: string;
  payarmed: string;
  payunarmed: string;
  expectarmed: string;
  expectunarmed: string;
  manname: string;
  addlcomm?: string;
  status: string;
  createdAt: string;
  notes?: string;
}

export default function AdminContracts() {
  const router = useRouter();
  const [contracts, setContracts] = useState<ContractSubmission[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState("");
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("All");
  const [selectedContract, setSelectedContract] = useState<ContractSubmission | null>(null);
  const [isUpdating, setIsUpdating] = useState<string | null>(null);
  const [isDeleting, setIsDeleting] = useState<string | null>(null);

  const fetchContracts = async () => {
    setIsLoading(true);
    setError("");
    try {
      const res = await fetch("/api/admin/contracts");
      if (!res.ok) throw new Error("Database server error.");
      const data = await res.json();
      setContracts(data.contracts || []);
    } catch (err: any) {
      setError(err.message || "Error loading records.");
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => {
    fetchContracts();
  }, []);

  const handleLogout = async () => {
    try {
      const res = await fetch("/api/admin/logout", { method: "POST" });
      if (res.ok) {
        router.push("/admin/login");
        router.refresh();
      }
    } catch (err) {
      console.error("Logout error:", err);
    }
  };

  const handleStatusChange = async (id: string, newStatus: string) => {
    setIsUpdating(id);
    try {
      const res = await fetch("/api/admin/contracts", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, status: newStatus }),
      });
      if (!res.ok) throw new Error("Failed to update status.");
      setContracts((prev) =>
        prev.map((sub) => (sub._id === id ? { ...sub, status: newStatus } : sub))
      );
      if (selectedContract?._id === id)
        setSelectedContract((prev) => (prev ? { ...prev, status: newStatus } : null));
    } catch (err: any) {
      alert(err.message || "Error updating state.");
    } finally {
      setIsUpdating(null);
    }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this contract opportunity?")) {
      return;
    }
    
    setIsDeleting(id);
    try {
      const res = await fetch(`/api/admin/contracts?id=${id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed to delete contract.");
      setContracts((prev) => prev.filter((sub) => sub._id !== id));
      if (selectedContract?._id === id) {
        setSelectedContract(null);
      }
    } catch (err: any) {
      alert(err.message || "Error deleting record.");
    } finally {
      setIsDeleting(null);
    }
  };

  const totalInquiries = contracts.length;
  const pendingCount = contracts.filter((s) => s.status === "Pending").length;
  const inProgressCount = contracts.filter((s) => s.status === "In Progress").length;
  const completedCount = contracts.filter((s) => s.status === "Completed").length;

  const filteredContracts = contracts.filter((sub) => {
    const matchesSearch =
      sub.company?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      sub.fromemail?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      sub.contactpreson?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = statusFilter === "All" || sub.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const navItems = [
    { id: "All", label: "All Contracts", count: totalInquiries, icon: LayoutDashboard },
    { id: "Pending", label: "Pending", count: pendingCount, icon: AlertCircle },
    { id: "In Progress", label: "In Progress", count: inProgressCount, icon: Clock },
    { id: "Completed", label: "Completed", count: completedCount, icon: CheckCircle },
  ];

  const statCards = [
    {
      label: "Total Contracts",
      value: totalInquiries,
      icon: Users,
      gradient: "from-[#eab308] to-[#e8c97a]",
      shadow: "shadow-[0_8px_24px_rgba(102,126,234,0.4)]",
    },
    {
      label: "Pending Review",
      value: pendingCount,
      icon: AlertCircle,
      gradient: "from-[#131e35] to-[#1a2845]",
      shadow: "shadow-[0_8px_24px_rgba(245,87,108,0.4)]",
    },
    {
      label: "In Progress",
      value: inProgressCount,
      icon: Activity,
      gradient: "from-[#131e35] to-[#1a2845]",
      shadow: "shadow-[0_8px_24px_rgba(79,172,254,0.4)]",
    },
    {
      label: "Completed",
      value: completedCount,
      icon: TrendingUp,
      gradient: "from-[#131e35] to-[#1a2845]",
      shadow: "shadow-[0_8px_24px_rgba(67,233,123,0.4)]",
    },
  ];

  return (
    <div className="flex min-h-screen font-[family-name:var(--font-barlow)] bg-[#0b1120]">
      <AdminSidebar />

      <div className="flex-1 min-w-0 flex flex-col">
        {/* Header */}
        <header className="bg-[#131e35]/[0.92] backdrop-blur-lg border-b border-[rgba(201,168,76,0.12)] py-3.5 px-8 sticky top-0 z-40">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3.5">
              <div className="bg-[#0b1120] rounded-[10px] px-4 py-1.5 flex items-center">
                <img
                  src="/images/logo2.png"
                  alt="VSF Admin Console"
                  className="h-[30px] w-auto object-contain"
                />
              </div>
              <div className="w-px h-6 bg-[#e2e8f0]" />
              <h1 className="font-[family-name:var(--font-bebas)] text-[22px] tracking-[2px] text-white m-0">
                {statusFilter === "All" ? "All Contracts" : statusFilter}
              </h1>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                <Search className="absolute left-3 top-[9px] h-[15px] w-[15px] text-[#94a3b8]" />
                <input
                  type="text"
                  placeholder="Search records..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-[240px] bg-[#1a2845] border-[1.5px] border-[rgba(201,168,76,0.2)] rounded-[10px] py-2 pl-9 pr-3.5 text-[13px] text-[#f4f6f8] font-[family-name:var(--font-barlow)] outline-none transition-all duration-200 focus:border-[#818cf8] focus:shadow-[0_0_0_3px_rgba(129,140,248,0.1)]"
                />
              </div>
              <button
                onClick={fetchContracts}
                className="flex items-center gap-1.5 py-2 px-4 rounded-[10px] border-[1.5px] border-[rgba(201,168,76,0.2)] bg-[#131e35] text-xs font-semibold text-[#cbd5e1] cursor-pointer transition-all hover:bg-[#1a2845]"
              >
                <RefreshCw className="h-3.5 w-3.5" /> Refresh
              </button>
              <button
                onClick={handleLogout}
                className="md:hidden flex items-center p-2 rounded-[10px] bg-red-50 border border-red-200 text-red-600 cursor-pointer"
              >
                <LogOut className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
        </header>

        {/* Body */}
        <div className="flex-1 p-7 animate-fade-up">
          {/* Stat Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-7">
            {statCards.map((card, i) => {
              const CardIcon = card.icon;
              return (
                <div
                  key={i}
                  className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${card.gradient} p-6 ${card.shadow} transition-all duration-300 hover:-translate-y-[3px]`}
                >
                  <div className="absolute -top-5 -right-5 w-[100px] h-[100px] rounded-full bg-[#131e35]/10" />
                  <div className="absolute -bottom-2.5 right-5 w-[60px] h-[60px] rounded-full bg-[#131e35]/[0.06]" />
                  <div className="relative z-[1]">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-xs font-semibold text-white/85 uppercase tracking-[1px]">
                        {card.label}
                      </span>
                      <div className="w-[38px] h-[38px] rounded-xl bg-[#131e35]/20 backdrop-blur-sm flex items-center justify-center">
                        <CardIcon className="h-[19px] w-[19px] text-white" />
                      </div>
                    </div>
                    <div className="text-4xl font-extrabold text-white leading-none font-[family-name:var(--font-bebas)] tracking-[2px]">
                      {card.value}
                    </div>
                  </div>
                </div>
              );
            })}
          </div>

          {/* Table Card */}
          <div className="bg-[#131e35] rounded-2xl overflow-hidden border border-[rgba(201,168,76,0.12)] shadow-[0_4px_16px_rgba(0,0,0,0.04)]">
            <div className="flex items-center justify-between px-6 py-[18px] border-b border-[rgba(201,168,76,0.15)]">
              <div className="flex items-center gap-2">
                <FileText className="h-4 w-4 text-[#eab308]" />
                <span className="text-sm font-bold text-white">Recent Contracts</span>
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-[6px] bg-[#eab308]/10 text-[#e8c97a]">
                  {filteredContracts.length}
                </span>
              </div>
              <div className="md:hidden flex gap-1">
                {navItems.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setStatusFilter(tab.id)}
                    className={`px-2.5 py-1 rounded-[6px] text-[10px] font-semibold ${
                      statusFilter === tab.id ? "bg-[#6366f1] text-white" : "bg-[#1a2845] text-[#8898aa]"
                    }`}
                  >
                    {tab.id === "All" ? "All" : tab.id}
                  </button>
                ))}
              </div>
            </div>

            {isLoading ? (
              <div className="py-20 text-center flex flex-col items-center gap-3.5">
                <div className="w-12 h-12 rounded-[14px] bg-gradient-to-br from-[#eab308] to-[#e8c97a] flex items-center justify-center shadow-[0_8px_20px_rgba(102,126,234,0.3)]">
                  <RefreshCw className="h-[22px] w-[22px] text-white animate-spin" />
                </div>
                <p className="text-sm text-[#8898aa] font-medium">Loading records...</p>
              </div>
            ) : error ? (
              <div className="py-20 text-center">
                <div className="w-12 h-12 rounded-[14px] bg-red-50 flex items-center justify-center mx-auto mb-3">
                  <AlertCircle className="h-[22px] w-[22px] text-red-500" />
                </div>
                <p className="text-sm font-semibold text-red-500">Failed to Load</p>
                <p className="text-xs text-[#94a3b8] mt-1">{error}</p>
              </div>
            ) : filteredContracts.length === 0 ? (
              <div className="py-20 text-center">
                <div className="w-16 h-16 rounded-[20px] bg-gradient-to-br from-[#e0e7ff] to-[#ede9fe] flex items-center justify-center mx-auto mb-4">
                  <FileText className="h-7 w-7 text-[#eab308]" />
                </div>
                <p className="text-[15px] font-bold text-white">No Records Found</p>
                <p className="text-[13px] text-[#94a3b8] mt-1">Contracts will appear here when received</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left">
                  <thead>
                    <tr className="bg-[#1a2845]">
                      {["Company", "Contact", "Date", "Location", "Status", "Actions"].map((h) => (
                        <th
                          key={h}
                          className={`py-3.5 px-[22px] text-[11px] font-bold tracking-[0.8px] uppercase text-[#94a3b8] border-b border-[rgba(201,168,76,0.15)] ${
                            h === "Actions" ? "text-right" : ""
                          }`}
                        >
                          {h}
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filteredContracts.map((sub) => {
                      const formattedDate = new Date(sub.createdAt).toLocaleDateString(undefined, {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      });
                      
                      const statusClass =
                        sub.status === "Completed"
                          ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                          : sub.status === "In Progress"
                          ? "bg-blue-50 text-blue-700 border-blue-200"
                          : "bg-amber-50 text-amber-700 border-amber-200";
                      
                      const dotClass =
                        sub.status === "Completed"
                          ? "bg-emerald-500"
                          : sub.status === "In Progress"
                          ? "bg-blue-500"
                          : "bg-amber-500";

                      return (
                        <tr key={sub._id} className="border-b border-[rgba(201,168,76,0.1)] hover:bg-[#1a2845]/50 transition-colors">
                          <td className="py-4 px-[22px]">
                            <div className="text-[13px] font-semibold text-[#f4f6f8]">{sub.company}</div>
                            <div className="text-[11px] text-[#94a3b8] mt-0.5">{sub.dba || "—"}</div>
                          </td>
                          <td className="py-4 px-[22px]">
                            <div className="text-[13px] font-medium text-[#cbd5e1]">{sub.contactpreson}</div>
                            <div className="text-[11px] text-[#e8c97a]">{sub.fromemail}</div>
                          </td>
                          <td className="py-4 px-[22px] text-xs text-[#8898aa]">{formattedDate}</td>
                          <td className="py-4 px-[22px] text-xs text-[#e2e8f0] font-medium">
                            {sub.city}, {sub.state}
                          </td>
                          <td className="py-4 px-[22px]">
                            <span className={`inline-flex items-center gap-[5px] px-3 py-1 rounded-full text-[11px] font-semibold border ${statusClass}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${dotClass}`} />
                              {sub.status}
                            </span>
                          </td>
                          <td className="py-4 px-[22px] text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                onClick={() => setSelectedContract(sub)}
                                className="inline-flex items-center gap-[5px] px-3.5 py-1.5 rounded-lg border-[1.5px] border-[rgba(201,168,76,0.2)] bg-[#131e35] text-xs font-semibold text-[#cbd5e1] hover:bg-[#1a2845] hover:border-[#eab308]"
                              >
                                <Eye className="h-[13px] w-[13px] text-[#eab308]" /> View
                              </button>
                              <select
                                disabled={isUpdating === sub._id}
                                value={sub.status}
                                onChange={(e) => handleStatusChange(sub._id, e.target.value)}
                                className="py-1.5 px-2.5 rounded-lg border-[1.5px] border-[rgba(201,168,76,0.2)] bg-[#1a2845] text-[11px] font-medium text-[#cbd5e1] outline-none"
                              >
                                <option value="Pending">Pending</option>
                                <option value="In Progress">In Progress</option>
                                <option value="Completed">Completed</option>
                              </select>
                              <button
                                onClick={() => handleDelete(sub._id)}
                                disabled={isDeleting === sub._id}
                                className="inline-flex items-center px-2.5 py-1.5 rounded-lg border-[1.5px] border-[#fee2e2] bg-[#131e35] text-[#ef4444] hover:bg-[#fef2f2] hover:border-[#fca5a5] disabled:opacity-50"
                              >
                                <Trash2 className="h-[13px] w-[13px]" />
                              </button>
                            </div>
                          </td>
                        </tr>
                      );
                    })}
                  </tbody>
                </table>
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Detail Modal */}
      {selectedContract && (
        <div className="fixed inset-0 bg-[rgba(15,15,30,0.5)] backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#131e35] w-full max-w-[800px] rounded-[20px] overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[90vh] animate-fade-up">
            <div className="flex justify-between items-center px-6 py-5 border-b border-[rgba(201,168,76,0.15)] bg-gradient-to-br from-[#131e35] to-[#1a2845]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#131e35]/15 backdrop-blur-sm flex items-center justify-center">
                  <Shield className="h-5 w-5 text-white" />
                </div>
                <div>
                  <h3 className="text-[15px] font-bold text-white m-0">Contract Details</h3>
                  <p className="text-[11px] text-white/50 mt-0.5">
                    {new Date(selectedContract.createdAt).toLocaleString()}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setSelectedContract(null)}
                className="w-[34px] h-[34px] rounded-[10px] border border-white/15 bg-[#131e35]/10 flex items-center justify-center text-white hover:bg-[#131e35]/20"
              >
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModalSection title="Company Details">
                  <DetailRow label="Company Name" value={selectedContract.company} />
                  {selectedContract.dba && <DetailRow label="DBA" value={selectedContract.dba} />}
                  <DetailRow label="Address" value={`${selectedContract.moaddress}, ${selectedContract.city}, ${selectedContract.state} ${selectedContract.zip}`} />
                  {selectedContract.website && <DetailRow label="Website" value={selectedContract.website} isLink />}
                </ModalSection>
                <ModalSection title="Contact Person">
                  <DetailRow label="Name" value={selectedContract.contactpreson} />
                  <DetailRow label="Email" value={selectedContract.fromemail} isLink />
                  <DetailRow label="Phone" value={selectedContract.ophone} />
                  {selectedContract.fax && <DetailRow label="Fax" value={selectedContract.fax} />}
                </ModalSection>
              </div>

              <div className="mt-6">
                <ModalSection title="Service Area">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <DetailRow label="States" value={selectedContract.stservice} />
                    <DetailRow label="Counties" value={selectedContract.coservice} />
                  </div>
                </ModalSection>
              </div>

              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModalSection title="Background">
                  <div className="p-3.5 bg-[#1a2845] rounded-[10px] border border-[rgba(201,168,76,0.2)] text-[13px] text-[#e2e8f0] whitespace-pre-wrap">
                    {selectedContract.bcbackground}
                  </div>
                </ModalSection>
                <ModalSection title="President Description">
                  <div className="p-3.5 bg-[#1a2845] rounded-[10px] border border-[rgba(201,168,76,0.2)] text-[13px] text-[#e2e8f0] whitespace-pre-wrap">
                    {selectedContract.bdpresident}
                  </div>
                </ModalSection>
              </div>

              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModalSection title="Services & Organization">
                  {selectedContract.services && selectedContract.services.length > 0 && (
                    <div className="mb-3 flex flex-wrap gap-1.5">
                      {selectedContract.services.map((svc) => (
                        <span key={svc} className="px-3 py-1.5 rounded-lg bg-[#eab308]/10 border border-[#c7d2fe] text-[11px] font-semibold text-[#4338ca]">
                          {svc}
                        </span>
                      ))}
                    </div>
                  )}
                  {selectedContract.orgtypetext && <DetailRow label="Other Services" value={selectedContract.orgtypetext} />}
                  {selectedContract.orgstruct && <DetailRow label="Structure" value={selectedContract.orgstruct} />}
                  {selectedContract.fein && <DetailRow label="FEIN" value={selectedContract.fein} />}
                </ModalSection>

                <ModalSection title="License & Insurance">
                  {selectedContract.ssle && <DetailRow label="State License" value={selectedContract.ssle} />}
                  {selectedContract.inscarrier && <DetailRow label="Carrier" value={selectedContract.inscarrier} />}
                  {selectedContract.insagent && <DetailRow label="Agent Name" value={selectedContract.insagent} />}
                  {selectedContract.insphone && <DetailRow label="Agent Phone" value={selectedContract.insphone} />}
                  {selectedContract.comliab && <DetailRow label="Liability Limit" value={selectedContract.comliab} />}
                </ModalSection>
              </div>

              <div className="mt-6">
                <ModalSection title="Business Practices">
                  <DetailRow label="Violations" value={selectedContract.voilation} />
                  <DetailRow label="Judgments" value={selectedContract.judgment} />
                  <DetailRow label="Recruiting Protocols" value={selectedContract.recruiting} />
                  <DetailRow label="Managers" value={selectedContract.manname} />
                </ModalSection>
              </div>

              <div className="mt-6">
                <ModalSection title="Rates">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <DetailRow label="Pay (Armed)" value={selectedContract.payarmed} />
                    <DetailRow label="Pay (Unarmed)" value={selectedContract.payunarmed} />
                    <DetailRow label="Expect (Armed)" value={selectedContract.expectarmed} />
                    <DetailRow label="Expect (Unarmed)" value={selectedContract.expectunarmed} />
                  </div>
                </ModalSection>
              </div>

              {selectedContract.addlcomm && (
                <div className="mt-6">
                  <ModalSection title="Additional Comments">
                    <div className="p-3.5 bg-[#1a2845] rounded-[10px] border border-[rgba(201,168,76,0.2)] text-[13px] text-[#8898aa] whitespace-pre-wrap">
                      {selectedContract.addlcomm}
                    </div>
                  </ModalSection>
                </div>
              )}
            </div>

            <div className="flex justify-end px-6 py-4 border-t border-[rgba(201,168,76,0.15)] bg-[#fafbfc]">
              <button
                onClick={() => setSelectedContract(null)}
                className="py-2.5 px-7 rounded-[10px] bg-gradient-to-br from-[#eab308] to-[#e8c97a] text-white text-[13px] font-bold shadow-[0_4px_14px_rgba(102,126,234,0.3)] hover:shadow-[0_8px_24px_rgba(102,126,234,0.4)]"
              >
                Close
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}

function ModalSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="text-[11px] font-bold tracking-[1.5px] uppercase text-[#eab308] mb-3.5 pb-2 border-b-2 border-[#eef2ff]">
        {title}
      </h4>
      <div className="flex flex-col gap-2.5">{children}</div>
    </div>
  );
}

function DetailRow({ label, value, isLink }: { label: string; value: string; isLink?: boolean }) {
  if (!value) return null;
  return (
    <div>
      <div className="text-[10px] font-bold tracking-[0.5px] uppercase text-[#94a3b8] mb-0.5">
        {label}
      </div>
      {isLink ? (
        <a href={label.includes("Email") ? `mailto:${value}` : value.startsWith("http") ? value : `https://${value}`} target="_blank" rel="noopener noreferrer" className="text-[13px] font-medium text-[#e8c97a] hover:underline">
          {value}
        </a>
      ) : (
        <div className="text-[13px] font-medium text-[#f4f6f8]">{value}</div>
      )}
    </div>
  );
}
