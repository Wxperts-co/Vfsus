    ...
    <div className="flex min-h-screen font-[family-name:var(--font-barlow)] bg-[#f0f2f7]">
          ...
          <div className="bg-white rounded-2xl overflow-hidden border border-black/5 shadow-[0_4px_16px_rgba(0,0,0,0.04)]">
            <div className="flex items-center justify-between px-6 py-[18px] border-b border-[#f1f5f9]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-[#1e1b4b]">Recent Contracts</span>
            ...
            {isLoading ? (
              ...
              <div className="py-20 text-center">
                ...
                <p className="text-[15px] font-bold text-[#1e1b4b]">No Records Found</p>
                      ...
                      return (
                        <tr key={sub._id} className="border-b border-[#f8fafc] hover:bg-[#f8faff] transition-colors">
                          <td className="py-4 px-[22px]">
                            <div className="text-[13px] font-semibold text-[#1e293b]">{sub.company}</div>
                            <div className="text-[11px] text-[#94a3b8] mt-0.5">{sub.dba || "—"}</div>
                          </td>
                          <td className="py-4 px-[22px]">
                            <div className="text-[13px] font-medium text-[#475569]">{sub.contactpreson}</div>
                            <div className="text-[11px] text-[#6366f1]">{sub.fromemail}</div>
                          </td>
                          <td className="py-4 px-[22px] text-xs text-[#64748b]">{formattedDate}</td>
                          <td className="py-4 px-[22px] text-xs text-[#334155] font-medium">
                            {sub.city}, {sub.state}
                          </td>
                          <td className="py-4 px-[22px]">
                          ...
                          <td className="py-4 px-[22px] text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="inline-flex items-center gap-[5px] px-3.5 py-1.5 rounded-lg border-[1.5px] border-[#e2e8f0] bg-white text-xs font-semibold text-[#475569] hover:bg-[#f1f5f9] hover:border-[#cbd5e1]"
                              >
                                <Eye className="h-[13px] w-[13px] text-[#818cf8]" /> View
                              </button>
                              <select
      ...
      {selectedContract && (
        <div className="fixed inset-0 bg-[rgba(15,15,30,0.5)] backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-[800px] rounded-[20px] overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[90vh] animate-fade-up">
            <div className="flex justify-between items-center px-6 py-5 border-b border-[#f1f5f9] bg-gradient-to-br from-[#1e1b4b] to-[#312e81]">
              ...
              <button
                ...
                className="w-[34px] h-[34px] rounded-[10px] border border-white/15 bg-white/10 flex items-center justify-center text-white hover:bg-white/20"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModalSection title="Background">
                  <div className="p-3.5 bg-[#f8fafc] rounded-[10px] border border-[#e2e8f0] text-[13px] text-[#334155] whitespace-pre-wrap">
                  ...
                  {selectedContract.services && selectedContract.services.length > 0 && (
                    <div className="mb-3 flex flex-wrap gap-1.5">
                      {selectedContract.services.map((svc) => (
                        <span key={svc} className="px-3 py-1.5 rounded-lg bg-[#eef2ff] border border-[#c7d2fe] text-[11px] font-semibold text-[#4338ca]">
                          {svc}
                        </span>
                      ))}
                    </div>
              ...
              {selectedContract.addlcomm && (
                <div className="mt-6">
                  <ModalSection title="Additional Comments">
                    <div className="p-3.5 bg-[#f8fafc] rounded-[10px] border border-[#e2e8f0] text-[13px] text-[#64748b] whitespace-pre-wrap">
                      {selectedContract.addlcomm}
                    </div>
                  </ModalSection>
                </div>
            ...
            <div className="flex justify-end px-6 py-4 border-t border-[#f1f5f9] bg-[#fafbfc]">
              <button
                onClick={() => setSelectedContract(null)}
                className="py-2.5 px-7 rounded-[10px] bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white text-[13px] font-bold shadow-[0_4px_14px_rgba(102,126,234,0.3)] hover:shadow-[0_8px_24px_rgba(102,126,234,0.4)]"
              >
                Close
              </button>
            </div>
...
function DetailRow({ label, value, isLink }: { label: string; value: string; isLink?: boolean }) {
  ...
  return (
    <div>
      ...
      {isLink ? (
        <a href={label.includes("Email") ? `mailto:${value}` : value.startsWith("http") ? value : `https://${value}`} target="_blank" rel="noopener noreferrer" className="text-[13px] font-medium text-[#6366f1] hover:underline">
          {value}
        </a>
      ) : (
